...
def send_commands_to_devices(devices,filename,show=None,config=None,limit=3):
    ...
    if show:
        with ThreadPoolExecutor(max_workers=limit) as executor:
            res1=executor.map(send_show,devices,repeat(show))
    if config:
        with ThreadPoolExecutor(max_workers=limit) as executor:
            res2=executor.map(send_config,devices,repeat(config))

    with open(filename,'w') as fwr:
        if res1:
            for device,output in zip(devices,res1):
                fwr.write(output+'\n')
        if res2:
            for device,output in zip(devices,res2):
                fwr.write(output+'\n')
