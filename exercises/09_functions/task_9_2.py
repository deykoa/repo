...
'''
def generate_trunk_config(trank_conf,trunk_temp):
    conf=[]
    for intf,vlans in trank_conf.items():
        conf.append('interface '+intf)
        for command in trunk_temp:
            if 'allowed vlan' in command:
                conf.append(command+' '+','.join([str(v) for v in vlans]))
            else: conf.append(command)
    return conf
