...
def convert_ranges_to_ip_list(lists_of_ranges):
    ips=[]
    for range_ip in lists_of_ranges:
        if '-' not in range_ip:
            ips.append(range_ip)
        else:
            ranges = range_ip.split('-')
            try:
                ipfirst=ranges[0].split('.')
                iplast=int(ranges[-1])
                for octet in range(int(ipfirst[-1]),int(iplast)+1):
                    ips.append(ipfirst[0]+'.'+ipfirst[1]+'.'+ipfirst[2]+'.'+str(octet))

            except:
                ipfirst=ranges[0].split('.')
                iplast=ranges[1].split('.')
                for octet in range(int(ipfirst[-1]),int(iplast[-1])+1):
                    ips.append(ipfirst[0]+'.'+ipfirst[1]+'.'+ipfirst[2]+'.'+str(octet))
    return ips


if __name__=='__main__':
    range_of_ip =['8.8.4.4', '1.1.1.1-3', '172.21.41.128-172.21.41.132']
    print(convert_ranges_to_ip_list(range_of_ip))
